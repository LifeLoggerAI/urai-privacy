
import json
import os
import subprocess
import requests
import sys

def get_api_key():
    """Gets the API key from the Firebase project configuration."""
    try:
        result = subprocess.run(
            ['firebase', 'apps:sdkconfig', 'WEB', '--project', 'urai-privacy', '--json'],
            capture_output=True,
            text=True,
            check=True
        )
        sdk_config = json.loads(result.stdout)
        return sdk_config['result']['apiKey']
    except (subprocess.CalledProcessError, KeyError, json.JSONDecodeError) as e:
        print(f"Error getting API key: {e}", file=sys.stderr)
        return None

def get_id_token(api_key, email, password):
    """Gets an ID token for a test user."""
    try:
        curl_command = [
            'curl',
            f'https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key={api_key}',
            '-H', 'Content-Type: application/json',
            '--data-binary', f'{{"email":"{email}","password":"{password}","returnSecureToken":true}}'
        ]
        result = subprocess.run(
            curl_command,
            capture_output=True,
            text=True,
            check=True
        )
        return json.loads(result.stdout)['idToken']
    except (subprocess.CalledProcessError, KeyError, json.JSONDecodeError) as e:
        print(f"Error getting ID token: {e}", file=sys.stderr)
        return None

def uid_spoofing_test(id_token, project_id, document_path):
    """
    Attempts to write to a Firestore document by forging a UID in the request body.
    """
    print(f"--- Running UID Spoofing Test ---")
    print(f"Attempting to write to document: '{document_path}' with a forged UID")
    print(f"Authenticating as user: 'testuser01@uraiprivacy.org'")

    try:
        # The user 'testuser01' attempts to write to a document belonging to 'testadmin01'
        # by forging the UID in the request body. The security rules should ignore the
        # forged UID and rely on the UID from the auth token.
        url = f"https://firestore.googleapis.com/v1/projects/{project_id}/databases/(default)/documents/{document_path}"
        headers = {
            "Authorization": f"Bearer {id_token}",
            "Content-Type": "application/json"
        }
        data = {
            "fields": {
                "forgedUid": {
                    "stringValue": "testadmin01"  # Attempt to forge the UID
                },
                 "someField": {
                    "stringValue": "malicious data"
                }
            }
        }
        response = requests.patch(url, headers=headers, data=json.dumps(data))

        print(f"Status Code: {response.status_code}")
        print(f"Response: {response.text}")

        if response.status_code == 403:
            print("\n✅   SUCCESS: Write was correctly forbidden by Firestore rules.")
        else:
            print(f"\n❌   FAILURE: Write was not forbidden. Unexpected status code: {response.status_code}")

    except Exception as e:
        print(f"An error occurred during the test: {e}", file=sys.stderr)
        print("\n❌   FAILURE: Test script encountered an error.")


if __name__ == "__main__":
    print("Starting simulated attack: UID Spoofing")
    api_key = get_api_key()
    if not api_key:
        sys.exit(1)

    attacker_email = "testuser01@uraiprivacy.org"
    attacker_pass = "password123"

    print(f"Getting auth token for user '{attacker_email}'...")
    id_token = get_id_token(api_key, attacker_email, attacker_pass)

    if not id_token:
        print("Could not obtain auth token. Aborting test.")
        sys.exit(1)

    print("Auth token obtained successfully.")

    project_id = "urai-privacy"
    # Document belonging to another user (testadmin01)
    target_document_path = "privacyRequests/testadmin01"

    uid_spoofing_test(id_token, project_id, target_document_path)

