...
def admin_privilege_escalation_test(id_token, project_id, admin_path):
    """
    Attempts to access a protected admin path as a regular user.
    """
    print(f"--- Running Admin Privilege Escalation Test ---")
    print(f"Attempting to access protected admin path: '{admin_path}'")
    print(f"Authenticating as user: 'testuser01@uraiprivacy.org'")

    try:
        url = f"https://firestore.googleapis.com/v1/projects/{project_id}/databases/(default)/documents/{admin_path}"
        headers = {
        ...
        if response.status_code == 403:
            print("\n✅   SUCCESS: Access to admin path was correctly forbidden by Firestore rules.")
        else:
            print(f"\n❌   FAILURE: Access to admin path was not forbidden. Unexpected status code: {response.status_code}")
...
if __name__ == "__main__":
    print("Starting simulated attack: Admin Privilege Escalation")
    api_key = get_api_key()
    if not api_key:
        sys.exit(1)
    ...
    # A hypothetical admin-only document
    protected_admin_path = "admin/someAdminDocument"

    admin_privilege_escalation_test(id_token, project_id, protected_admin_path)
