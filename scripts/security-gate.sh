...
require_pattern "functions/src/deletion-mutation-guard.ts" 'deletionCompletionAuthorityBlockReason' "completion authority policy"
require_pattern "functions/src/deletion-mutation-guard.ts" 'requireCompletionAuthority\(snapshot\.data\(\) \?\? [{]}, args\)' "transactional completion authority check"
require_pattern "functions/src/deletion-mutation-guard.ts" 'leaseToken: token' "exact verifier lease propagation"
require_pattern "functions/src/deletion-completion-authority.ts" 'state\.uid !== input\.targetUid' "target UID stability check"
require_pattern "functions/src/deletion-completion-authority.ts" 'state\.deletionMutationLeaseToken !== input\.leaseToken' "exact lease token check"
...
require_pattern "tests/unit/deletion-completion-authority.test.ts" 'rejects target UID drift' "UID drift regression test"
require_pattern "tests/unit/deletion-completion-authority.test.ts" 'rejects a superseded lease token' "lease supersession regression test"
require_pattern "tests/unit/deletion-completion-authority.test.ts" 'rejects an expired or malformed lease' "lease expiry regression test"
require_pattern "tests/unit/deletion-completion-authority.test.ts" 'rejects verification that is no longer pending' "verification-state regression test"
