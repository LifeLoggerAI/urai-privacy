...
if (process.env.URAI_PRIVACY_RELEASE_SHA && !/^[0-9a-f]{7,40}$/i.test(process.env.URAI_PRIVACY_RELEASE_SHA)) {
...
const missingControls = optionalEvidenceEnv.filter((name) => yesNo(process.env[name]) !== "yes");
if (requireLive && (missingRequired.length > 0 || missingControls.length > 0)) {
  if (missingRequired.length > 0) console.error(`[staging-evidence] missing required env: ${missingRequired.join(", ")}`);
  if (missingControls.length > 0) console.error(`[staging-evidence] incomplete controls: ${missingControls.join(", ")}`);
  process.exit(1);
}

if (missingRequired.length > 0 || missingControls.length > 0) {
