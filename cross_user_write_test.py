...
def cross_user_write_test(id_token, project_id, document_path):
    """
    Attempts to write to a Firestore document using the provided ID token.
    """
    print(f"--- Running Cross-User Write Test ---")
    print(f"Attempting to write to document: '{document_path}'")
    ...
    try:
        ...
        # Attempt to overwrite the document with new data
        data = {
            "fields": {
                "someField": {
...
if __name__ == "__main__":
    print("Starting simulated attack: Cross-User Document Write")
    api_key = get_api_key()
    if not api_key:
        sys.exit(1)
    ...
    cross_user_write_test(id_token, project_id, target_document_path)
