...
def collection_scan_test(id_token, project_id, collection_id):
    """
    Attempts to list all documents in a Firestore collection.
    """
    print(f"--- Running Mass Collection Scan Test ---")
    print(f"Attempting to scan collection: '{collection_id}'")
    ...
    try:
        ...
        if response.status_code == 403:
            print("\n✅   SUCCESS: Collection scan was correctly forbidden by Firestore rules.")
        else:
            print(f"\n❌   FAILURE: Collection scan was not forbidden. Unexpected status code: {response.status_code}")
...
if __name__ == "__main__":
    print("Starting simulated attack: Mass Collection Scan")
    api_key = get_api_key()
    if not api_key:
        sys.exit(1)
    ...
    collection_scan_test(id_token, project_id, target_collection_id)
