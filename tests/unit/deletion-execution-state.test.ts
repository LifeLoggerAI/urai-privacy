...
test("the verification transaction is the sole final completion writer", () => {
  ...
  assert.match(source, /const tombstoneRef = db\.collection\("privacyDeletionTombstones"\)\.doc\(uid\)/);
  assert.match(source, /tx\.set\(tombstoneRef,/);
  assert.match(source, /status: "deletion_in_progress"/);
  assert.match(source, /expectedPlanHash !== approvedPlanHash[\s\S]*tx\.set\(tombstoneRef,/);
