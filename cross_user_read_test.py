...
def cross_user_read_test(id_token, project_id, document_path):
    """
    Attempts to read a Firestore document using the provided ID token.
    """
    print(f"--- Running Cross-User Read Test ---")
    print(f"Attempting to read document: '{document_path}'")
    ...
    try:
        ...
        if response.status_code == 403:
            print("\n✅   SUCCESS: Read was correctly forbidden by Firestore rules.")
        else:
            print(f"\n❌   FAILURE: Read was not forbidden. Unexpected status code: {response.status_code}")
...
if __name__ == "__main__":
    print("Starting simulated attack: Cross-User Document Read")
    api_key = get_api_key()
    if not api_key:
        sys.exit(1)
    ...
    cross_user_read_test(id_token, project_id, target_document_path)
